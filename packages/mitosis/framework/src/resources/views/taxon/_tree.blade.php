@foreach($taxons as $taxon)
    <div class="card-block">
        ...
        @can('edit taxons')
            <a href="{{ route('mitosis.taxon.edit', [$taxonomy, $taxon]) }}">{{ $taxon->name }}</a>
        @else
            {{ $taxon->name }}
        @endcan
        ...
        <div class="card-actionbar card-actionbar-show-on-hover">
            @can('create taxons')
                <a href="{{ route('mitosis.taxon.create', $taxonomy) }}?parent={{$taxon->id}}"
            ...
            @can('delete taxons')
                {{ Form::open([
                            'url' => route('mitosis.taxon.destroy', [$taxonomy, $taxon]),
    ...
    @if ($taxon->children->isNotEmpty())
        <div class="collapse multi-collapse" id="taxon-{{$taxon->id}}" data-toggle="collapse">
            <div class="card-block">
                <div class="card">
                    @include('mitosis::taxon._tree', ['taxons' => $taxon->children])
                </div>
            </div>
        </div>
    @endif
